...
function downloadExcelFileTypes({
  ...
  try {
    const json = generateExcelJSON({
      ...
      data,
      orgUnits,
      dataSources: flatten(dataHolders?.map(({ dataSources }:any) => dataSources)),
    });
...
export function downloadExcel({ periods, data, orgUnits, dataHolders, title }:any) {
...
export function getALMAMetadata({
  ...
  function getMetadataDimensions(orgUnits:any, dataSources:any, periods:any) {
    return {
  ...
  function getAllLegends(legends:any) {
    if (Array.isArray(legends)) {
      ...
      forIn(legends, (value:any, key:any) => {
...
export function downloadALMAMeta({ orgUnits, dataHolders, title }:any) {
  try {
    const json = JSON.stringify({
      organisationUnits: orgUnits?.map(({ id, level, displayName, path }:any) => {
...
export function downloadCSV({ periods, data, orgUnits, dataHolders, title }:any) {
  return downloadExcelFileTypes({
    periods,
    data,
    orgUnits,
