...
function SingleAverageCell({dataSources, values, bold}:any) {

    const [dataSource] = dataSources ?? [];
    const defaultLegendDefinitions = useRecoilValue<any>(
...
function LinkedAverageCell({dataSources, values, bold}:any) {
    const defaultLegendDefinitions:any = useRecoilValue(
        ScorecardLegendDefinitionSelector(true)
    );
    const orgUnitLevels = useRecoilValue(OrgUnitLevels);
    const legendDefinitions = useRecoilValue(
...
function ComplexAverageCell({value, bold, dataSources}:any) {
...
export default function AverageCell({value, bold, dataSources}:any) {
    if (value === undefined) {
        return null;
    }
