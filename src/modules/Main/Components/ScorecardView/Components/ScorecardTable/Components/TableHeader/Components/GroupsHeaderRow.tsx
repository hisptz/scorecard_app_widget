...
export default function GroupsHeaderRow({ nested, orgUnits }:any):React.ReactElement {
  const { dataGroups } =
  ...
  const onSortIconClick = ({ direction }:any) => {
    setSort((prevValue:any) => ({ ...prevValue, orgUnit: direction }));
  };
  ...
  return (
    <DataTableRow>
      ...
      {dataGroups?.map(({ title, id, dataHolders }:any) => (
        <DataTableCell
          fixed
