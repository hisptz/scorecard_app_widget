...
export default function OrgUnitHeaderRow({ orgUnits, nested }:any):React.ReactElement {
  ...
  const [{ name, direction }, setDataSort] = useRecoilState<any>(
    ScorecardTableSortState
  );
  ...
  const onSortIconClick = ({ direction }:any) => {
    setSort((prevValue:any) => ({ ...prevValue, data: direction }));
  };
